...
impl<C: VMHooksContext> VMHooks for VMHooksDispatcher<C> {
    ...
    fn get_owner_address(&mut self, result_offset: MemPtr) -> Result<(), VMHooksEarlyExit> {
        self.handler
            .use_gas(self.handler.gas_schedule().base_ops_api_cost.get_owner_address)?;
        ...
        unsafe {
            self.handler.context.memory_store(result_offset, &owner.to_vec());
        }
    ...
    fn write_event_log(
        ...
        self.handler.use_gas(self.handler.gas_schedule().base_ops_api_cost.log)?;
        ...
        let topic_lengths_bytes =
            unsafe { self.handler.context.memory_load(topic_lengths_offset, topic_lengths_len) };
        ...
        self.handler.context.push_tx_log(crate::host::context::TxLog {
    ...
    fn keccak256(
        ...
        unsafe {
            self.handler.context.memory_store(result_offset, &result[..]);
        }
