...
///
/// On non-wasm targets, this function is a no-op that returns `0`.
#[inline]
...
pub fn require_valid_aml_status<M: ManagedTypeApi>(status: &ManagedBuffer<M>) {
    let bytes = status.to_boxed_bytes();
    let s = bytes.as_slice();
    let allowed: &[&[u8]] = &[b"clear", b"flagged", b"review", b"blocked", b"not_started", b"deactivated"];
    if !allowed.iter().any(|a| *a == s) {
        M::error_api_impl().signal_error(
            b"invalid aml_status: must be one of clear, flagged, review, blocked, not_started, deactivated",
        );
    }
}
...
/// Represents the per-holder, per-token compliance state mirrored off-chain.
#[type_abi]
#[derive(TopEncode, TopDecode, NestedEncode, NestedDecode, ManagedVecItem, Clone)]
...
/// Represents the per-holder identity profile mirrored off-chain.
#[type_abi]
#[derive(TopEncode, TopDecode, NestedEncode, NestedDecode, ManagedVecItem, Clone)]
...
/// Represents the mirrored auditor authorization state for a holder.
#[type_abi]
#[derive(TopEncode, TopDecode, NestedEncode, NestedDecode, ManagedVecItem, Clone)]
...
/// guard. DRWA contracts that need governance access control should inherit
/// this trait as a supertrait.
...
// NOTE: All four DRWA contracts (identity-registry, policy-registry, asset-manager,
// attestation) share an identical sync-emit pattern: hash payload → invoke hook →
// verify result → return envelope. A future refactor should extract this into a
// shared trait method here, requiring CryptoApi + HandleConstraints bounds on the
// managed type. See RO-01 in the audit tracker.
