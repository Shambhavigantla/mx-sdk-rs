...
pub trait BufferPool: mrv_common::MrvGovernanceModule {
    /// Initializes the buffer pool with governance and carbon-credit contract addresses.
    #[init]
    ...
    fn replenish_buffer_credits(
        ...
        // Check cooldown if any replenishment has occurred (total_replenished > 0 covers epoch-0 case)
    ...
    fn require_authorized_caller(&self) {
        ...
        require!(is_governance || is_owner || is_carbon_credit, "caller not authorized");
    }
