...
pub trait MrvRegistry: mrv_common::MrvGovernanceModule {
    ...
    ///  - `quantity` is retained as an opaque `ManagedBuffer` (current
    ///    callers use a fixed-point decimal like `"10.5000"`). Numeric
    ///    parsing and `reversed_amount <= quantity` enforcement are
    ///    tracked as sub-finding B-01b pending an explicit format
    ///    convention in the canonical evolution plan.
    ...
    fn create_issuance_lot(
        ...
        require!(
            self.verification_cases().contains_key(&verification_case_id),
            "ENTITY_NOT_FOUND: verification_case"
        );
