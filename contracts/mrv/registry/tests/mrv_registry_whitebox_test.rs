...
fn mrv_registry_accepts_full_replacement_lineage() {
    ...
    world
        ...
        .whitebox(mrv_registry::contract_obj, |sc| {
            ...
            assert_eq!(original.reversed_amount.to_boxed_bytes().as_slice(), b"2.0000");
            let replacement = sc
                .get_issuance_lot(ManagedBuffer::from(b"lot-replacement"))
                .into_option()
                .unwrap();
            assert_eq!(
                replacement.replacement_for_lot_id.to_boxed_bytes().as_slice(),
                LOT_ID
            );
        });
}
